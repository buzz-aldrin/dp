...
no matter how we parenthesize the product, the result will be the same. For example, if we had four matrices
...
compute the product, or the efficiency. For example, suppose A is a 10 × 30 matrix, B is a 30 × 5 matrix, and
C is a 5 × 60 matrix. Then,

    (AB)C = (10×30×5) + (10×5×60) = 1500 + 3000 = 4500 operations
    A(BC) = (30×5×60) + (10×30×60) = 9000 + 18000 = 27000 operations.

Given an array input[] which represents the chain of matrices such that the ith matrix Ai is of dimension p[i-1] x p[i].
We need to write a function MatrixChainOrder() that should return the minimum number of multiplications needed to
multiply the chain.
*/

var (
	input = []int{40, 20, 30, 10, 30}
)

func main() {
	fmt.Println(mco(input, 1, len(input)-1))
}

func mco(input []int, x, y int) int {
	if x == y {
		return 0
	}
	...
	for k := x; k < y; k++ {
		temp := mco(input, x, k) + mco(input, k+1, y) + (input[x-1] * input[k] * input[y])
		if temp < count {
			count = temp
		}
	}
